    ...
    <div class="min-h-full">
        ...
        <main class="relative">
            <div class="text-center mx-16 my-6 p-6 max-w-lg shadow bg-white border border-gray-200 rounded-lg">
                ...
                <form
                action="{{route('destroy', [ 'username' => $username, 'id' => $property['id'] ])}}"
                method="POST">
                    @csrf
                    @method('DELETE')
                    <div class="mx-auto w-80 mx-2 mt-6">
                        ...
                        <div class="mt-1">
                            <div class="flex rounded-md shadow-sm bg-white ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                                <input type="text" name="title" id="title" value="{{ old('title') }}" class="block flex-1 border-0 bg-transparent py-1.5 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" placeholder="Property title to confirm deletion">
                            </div>
                        </div>
