...
<aside id="menu" class="fixed right-0 z-10 hidden flex float-right shadow-xl">
    <div class="text-gray-300 h-screen py-6 flex flex-col items-center overflow-y-auto bg-gray-800 border-gray-100 border-l sm:w-64 w-60">
        ...
        <div class="w-full px-2 text-">
            <a href="/{{ $user->username }}/properties" class="{{ $curUrl==$baseUrl . "/$user->username/properties" ? $activeStyles : $defaultStyles; }} flex items-center gap-2 my-1 p-1.5 focus:outline-nones transition-colors duration-200 rounded-lg">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                </svg>
                <h5>My Properties</h5>
            </a>
            <a href="/{{ $user->username }}" class="{{ $curUrl==$baseUrl . "/$user->username" ? $activeStyles : $defaultStyles; }} flex items-center gap-2 my-1 p-1.5 focus:outline-nones transition-colors duration-200 rounded-lg">
